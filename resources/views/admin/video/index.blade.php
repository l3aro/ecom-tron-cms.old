...
<div class="containter-fluid">
    ...
    <div class="row mt-2 mb-2">
        <div class="col-md-6 text-left">
            <a class="btn btn-sm btn-info" href="/admin/video/detail" data-toggle="tooltip" title="Thêm sản phẩm mới"><i class="material-icons">note_add</i></a>
    ...
    <div class="table-responsive" id="cat_table">
        @include('admin/video/list_video')
    </div>
</div>
<script>
    $(document).ready(function(){
        $('[data-toggle="tooltip"]').tooltip();
        $('#menu-list-video').parent('li').addClass('active');        
	...
	function init_row_action() {
		...
		$('.delete-button').click(function(e){
			...
			if (confirm('Bạn có chắc chắn muốn xóa?')){
				...
				$.ajax({
					url: '/admin/video/delete/' + delete_id,
					async: true,
			...
			if (countchecked > 0){
				if (confirm('Bạn có chắc chắn muốn xóa?')){
					...
					$('input.checkdel:checkbox:checked').each(function(index){
						$.ajax({
							url: '/admin/video/delete/' + $(this).attr('del-id'),
							async: false,
							success: function(data) {
								if (data != 0) {
									alert(data);
		...
		$( ".sort" ).sortable({
			...
			placeholder: "ui-state-highlight",
	      	update: function(event, ui) {
                ...
                $.post('/admin/video/sort', {sort: sort, _token : $('input[name=_token]').val()});
            }
	    });
        $('button[cms-change-field="changfield"]').click(function(){
			...
			$.ajax({
				  url: '/admin/video/changfield?id=' + id + '&p=' + currentvalue + '&field=' + field,
				  success: function(data) {
				  	if (currentvalue==0) { 
				  		pic = 'check';
				  		currentvalue = 1;
    ...
    function run_search(page){
        ...
        $.ajax({
            url: '/admin/video?keyword=' + keyword + '&cat=' + cat + '&user=' + user + '&from_date=' + fromDate + '&toDate=' + toDate + '&page=' + page,
            type: 'GET',
            success: function(data) {
                $('#cat_table').html(data);
                init_row_action();
