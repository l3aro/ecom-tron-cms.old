...
<div class="containter-fluid">
    ...
    <div class="row mt-2 mb-2">
        <div class="col-md-6 text-left">
            <a class="btn btn-sm btn-info" href="/admin/product/detail" data-toggle="tooltip" title="Thêm sản phẩm mới"><i class="material-icons">note_add</i></a>
...
<script>
    $(document).ready(function(){
        ...
        $('#menu-list-product').parent('li').addClass('active');
	...
	function init_row_action() {
			...
			if (countchecked > 0){
				if (confirm('Bạn có chắc chắn muốn xóa?')){                   
					...
					$('input.checkdel:checkbox:checked').each(function(index){
						$.ajax({
							url: '/admin/product/delete/' + $(this).attr('del-id'),
							async: false,
							success: function(data) {
								if (data != 0) {
									alert(data);
		...
		$( ".sort" ).sortable({
			...
			placeholder: "ui-state-highlight",
	      	update: function(event, ui) {
                ...
                $.post('/admin/product/sort', {sort: sort, _token : $('input[name=_token]').val()});
            }
	    });
		$('.move-top-button').click(function(e){
            ...
            $('#cat_table').html(showloading());
            $.ajax({
                url: '/admin/product/movetop/' + product_id + '/' + cat_id,
                async: true,
                success: function(data) {
                    if (data != 0) {
                        alert(data);
    ...
    function run_search(page){
        ...
        $.ajax({
            url: '/admin/product?keyword=' + keyword + '&cat=' + cat + '&user=' + user + '&from_date=' + fromDate + '&toDate=' + toDate + '&page=' + page,
            type: 'GET',
            success: function(data) {
                $('#cat_table').html(data);
                init_row_action();
