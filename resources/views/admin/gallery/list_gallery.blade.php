<table class="table-sm table-hover table-bordered mb-2" id="table_content" width="100%">
	<thead class="thead-light">
		<tr>
			...
			<th class="align-middle">Tên mục album</th>
	...
	@foreach ($gallery as $key=>$r)
		<tr id="item_<?=$r->id?>">
			...
			<td><?=$r->id?></td>
			<td class="edit-name">
				<a href="/admin/gallery/detail?id=<?=$r->id?>"><?=$r->name?></a>
			</td>
			<td>
				<a href="/admin/gallerycat/detail?id=<?=$r->cat?>"><?=$r->cat==0?'':$r->gallery_cat->name?></a>
			</td>
			<td>
			...
			<td><?=$r->updated_at?></td>
			<td><?=isset($r->galleryUpdateBy)?$r->galleryUpdateBy->name:''?></td>
			<td style="text-align: center;">
				<div class="btn-group">
					<a class="btn btn-sm p-1 btn-info" href="/admin/gallery/detail?id=<?=$r->id?>" data-toggle="tooltip" title="Sửa">
						<i class="material-icons">border_color</i>
					</a>
					<a class="btn btn-sm p-1 btn-success editmenu" href="/admin/gallery/copy?id=<?=$r->id?>" data-toggle="tooltip" title="Copy dữ liệu">
						<i class="material-icons">content_copy</i>
					</a>
					<a class="btn btn-sm p-1 btn-warning move-top-button" gallery-id="<?=$r->id?>" data-toggle="tooltip" title="Đưa lên đầu tiên">
