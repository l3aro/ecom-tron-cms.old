...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-9">
            <h1>Quản lý danh mục album ảnh</h1>
        </div>
	...
	<div class="row">
		<div class="col">
				<a href="/admin/gallerycat/detail" class="btn btn-sm btn-info pull-right my-2">
	...
	<div class="table-responsive" id="cat_table">
		@include('admin/gallerycat/list_categories')
	</div>
</div>
<script>
	...
	$(document).ready(function(){
		...
		$('#menu-list-cat-gallery').parent('li').addClass('active');
		init_row_action();
	});
	...
	function init_row_action() {
			...
			if (countchecked > 0){
				if (confirm('Bạn có chắc chắn muốn xóa?')){
					...
					$('input.checkdel:checkbox:checked').each(function(index){
						$.ajax({
							url: '/admin/gallerycat/delete/' + $(this).attr('del-id'),
							async: false,
							success: function(data) {
								if (data != 0) {
									alert(data);
		...
		$( ".sortcat" ).sortable({
	        ...
	        $.post('/admin/gallerycat/sortcat', {sort: sort, _token : $('input[name=_token]').val()});
	      }
	    });
	}
</script>
