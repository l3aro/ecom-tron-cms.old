...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/media/products/tmp/',
            'upload_url' => $this->get_full_url().'/media/products/tmp/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            'image_versions' => array(
                ...
                */
                'thumbnail' => array(
                    ...
                    'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/media/products/tmp/tb/',
                    'upload_url' => $this->get_full_url().'/media/products/tmp/tb/',
    ...
    protected function create_scaled_image($file_name, $version, $options) {
        ...
        switch (strtolower(substr(strrchr($file_name, '.'), 1))) {
            ...
            case 'gif':
                ...
                break;
            case 'png':
                $resizedImage = imagecreatetruecolor($img_width, $img_height);
                ImageAlphaBlending($resizedImage,false);
                ImageSaveAlpha($resizedImage,true);
                imagecopyresampled($resizedImage, $new_img, 0, 0, 0, 0, $img_width,	$img_height, $img_width, $img_height);

                $dest_image = imagecreatetruecolor($new_width, $new_height);
                ImageAlphaBlending($dest_image,false);
                ImageSaveAlpha($dest_image,true);
                imagecopyresampled($dest_image, $resizedImage, 0, 0, $dst_x, $dst_y, $new_width,$new_height, $new_width, $new_height);

                //imagepng($dest_image, $output_filename);
    ...
    function get_config_bytes($val) {
        ...
        switch($last) {
            case 'g':
                $valuelconvert = (float)(str_replace($val[strlen($val)-1], '', $val)) * 1024;
                $val = $valuelconvert .  $val[strlen($val)-1];
            case 'm':
                $valuelconvert = (float)(str_replace($val[strlen($val)-1], '', $val)) * 1024;
                $val = $valuelconvert .  $val[strlen($val)-1];
