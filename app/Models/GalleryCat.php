...
class Gallerycat extends Model
{
	...
	public function gallery() {
        return $this->hasMany('App\Models\gallery', 'cat');
	}
	public function childCats() {
		return $this->hasMany('App\Models\Gallerycat', 'parent', 'id');
	}
	public function allChildCats() {
		return $this->childCats()->with('allChildCats');
	}
    ...
    public function GetOptions( $active_id = 0){
        ...
        $cats = self::where('parent', 0)->where('language', session('lang'))->orderBy('order', 'desc')->get();
		foreach ($cats as $r) {
	...
	function GetCategories($lang = ''){
		...
		foreach ($results as $r) {
			...
			$this->_menus .= '<tr>
								...
								<td class="edit-name">
									<a href="/admin/gallerycat/detail?id='.$r->id.'">
										'.$r->name.'
									</a>
								</td>
								<td style="width: 120px;">'.$r->created_at.'</td>
								<td style="width: 120px;">'.$r->galleryUpdateBy->name.'</td>
								<td style="width: 115px;">
									<div class="btn-group">
										<a class="btn btn-success btn-sm p-1" href="/admin/gallerycat/detail?parent='.$r->id.'" data-toggle="tooltip" title="Thêm mục con">
											<i class="material-icons">playlist_add</i>
										</a>
										<a class="btn btn-info btn-sm p-1" href="/admin/gallerycat/detail?id='.$r->id.'" data-toggle="tooltip" title="Sửa">
											<i class="material-icons">mode_edit</i>
										</a>
										<a class="btn btn-danger btn-sm p-1 delete-button" href="/admin/gallerycat/delete/'.$r->id.'"
		...
		if(count($cats)){
			...
			foreach ($cats as $cat){
				...
				$this->_menus .= '<tr>
									...
									<td class="edit-name">
										<a href="/admin/gallerycat/detail?id='.$cat->id.'">
											'.$icon.$cat->name.'
										</a>
									</td>
									<td style="width: 120px;">'.$cat->created_at.'</td>
									<td style="width: 120px;">'.$cat->galleryUpdateBy->name.'</td>
									<td style="width: 115px;">
										<div class="btn-group">
											<a class="btn btn-success btn-sm p-1" href="/admin/gallerycat/detail?parent='.$cat->id.'" data-toggle="tooltip" title="Thêm mục con" data-placement="top">
												<i class="material-icons">playlist_add</i>
											</a>
											<a class="btn btn-info btn-sm p-1" href="/admin/gallerycat/detail?id='.$cat->id.'" data-toggle="tooltip" title="Sửa" data-placement="top">
												<i class="material-icons">mode_edit</i>
											</a>
											<a class="btn btn-danger btn-sm p-1 delete-button" href="/admin/gallerycat/delete/'.$cat->id.'"
